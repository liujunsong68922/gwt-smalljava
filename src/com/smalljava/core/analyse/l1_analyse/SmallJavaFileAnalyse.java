...
import com.smalljava.core.analyse.l1_analyse.manager.SmallJavaFileAnalysePluginManager;
...
public class SmallJavaFileAnalyse {
	private Logger logger = LoggerFactory.getLogger(SmallJavaFileAnalyse.class);
	...
	public JavaFileRootVO analyse(String strcontent) {
		...
		SmallJavaFileAnalysePluginManager manager = new SmallJavaFileAnalysePluginManager();
		...
		while (strcontent.length() > 0) {
			...
			for (ISmallJavaFileAnalysePlugin plugin : manager.getPluginarray()) {
