...
public class LogicGreaterEqualOperEvalPlugin implements ISmallJavaExpressionEval {
	private Logger logger = LoggerFactory.getLogger(LogicGreaterEqualOperEvalPlugin.class);
	@Override
		...
		if (root instanceof DualOperDataOperElement) {
			...
			if (oper.getOpercode().equals(">=")) {
				...
				if (leftvar == null || rightvar == null) {
					logger.error("leftvar or rightvar is Ϊnull");
					return null;
				}
				...
				if (leftvar.getVartype().equals("int")) {
					...
					boolean b1 = intoper.doGE(rightvar.getVarsvalue());
				...
				if (leftvar.getVartype().equals("long")) {
					...
					boolean b2 = longoper.doGE(rightvar.getVarsvalue());
				...
				if (leftvar.getVartype().equals("float")) {
					...
					boolean b3 = floatoper.doGE(rightvar.getVarsvalue());
				...
				if (leftvar.getVartype().equals("double")) {
					...
					boolean b4 = doubleoper.doGE(rightvar.getVarsvalue());
				...
				logger.error("[error] unsupport type:" + leftvar.getVartype());
				return null;
