...
/**
 * Object对应的评估器，针对Object进行评估
 * 
 * @author liujunsong
 *
 */
public class SmalljavaObjectEvalutor {
	Logger logger = LoggerFactory.getLogger(SmalljavaObjectEvalutor.class);

	/**
	 * MEMO：从对象实例的变量表中检索对应的属性值返回。如果找不到这个属性值，则返回null值
	 * 
	 * @param classvo
	 * @param propertyname
	 * @return
	 */
	public VarValue objectPropertyGet(JavaClassInstanceVO objectvo, String propertyname) {
		IVarTable vartable = objectvo.getVartable();
	 ...
	 * @return
	 */
	public Object objectmethod(JavaClassInstanceVO objectvo, 
			String methodname, 
			ArrayList<VarValue> arglist,
			SmallJavaClassSupportEnv classenv, 
			SmallJavaOopSupportEnv oopenv) {
		
		SmallJavaClassTemplateVO classvo = objectvo.getClasstemplatevo();
		SmallJavaClassMethodElement method = this._getClassMethod(classvo , methodname, arglist);
		if (method == null) {
	 ...
	 * @return
	 */
	private SmallJavaClassMethodElement _getClassMethod(SmallJavaClassTemplateVO classvo, String methodname,
			ArrayList<VarValue> arglist) {

		ArrayList<SmallJavaClassMethodElement> objectmethodlist = new ArrayList<SmallJavaClassMethodElement>();
		for (AbstractSmallJavaClassElement child : classvo.getChildren()) {
			if (child instanceof SmallJavaClassMethodElement) {
				...
				objectmethodlist.add(method);
			}
		}

		if (objectmethodlist.size() == 0) {
			// 未找到符合条件的Method定义
			return null;
		}
		...
		// 循环判断方法是否符合条件
		for (SmallJavaClassMethodElement method1 : objectmethodlist) {
	 ...
	 * @param classvo
	 */
	public boolean objectInitVartable(JavaClassInstanceVO instancevo, SmallJavaClassSupportEnv classenv,
			SmallJavaOopSupportEnv oopenv) {
		
		
		SmallJavaClassTemplateVO classvo = instancevo.getClasstemplatevo();
		// 定义不代表static 变量定义的语句
		String strNoStaticDefine = "";
		for (AbstractSmallJavaClassElement child : classvo.getChildren()) {
			if (child instanceof SmallJavaClassVarDefineElement) {
				...
				if (! (define.getStringcontent().contains("static "))) {
					strNoStaticDefine += define + ";";
				}
			}
		}
		logger.info("[info]strStaticDefine: " + strNoStaticDefine);
		if (strNoStaticDefine.length() == 0) {
			return true;
		}

		// 分析方法字符串,使用BlockAnalyse
		BasicBlock closedblock = new BasicBlock("", strNoStaticDefine, null);
		BlockAnalyse ba = new BlockAnalyse();

		boolean isok = ba.analyse(closedblock);
		if (!isok) {
			// analyse block failed.
			logger.error("[ERROR] Call block analyse failed." + strNoStaticDefine);
			return false;
		} else {
			// analyse block success.
			SmallJavaBlockEvaluator blockeval = new SmallJavaBlockEvaluator();
	 ...
	 * MEMO：调用类定义的构造函数
	 * TODO：找到构造函数的定义并进行调用
	 * MEMO:暂时不考虑带参数的构造函数的问题
	 * @param classvo
	 */
	public boolean objectConstructor(JavaClassInstanceVO instancevo, SmallJavaClassSupportEnv classenv,
			SmallJavaOopSupportEnv oopenv) {
		//TODO:支持构造函数的定义
		logger.info("[info]Constructor is not supported currently.");
		return true;
	}	
}
